...
function renderList(parks) {
    ...
    container.innerHTML = parks.map(p => {
        const imageSrc = (p.park_images && p.park_images.length > 0)
            ? buildImagePath(p.id, p.park_images[0])
            : 'https://via.placeholder.com/150?text=No+Image';

        const district = p.district && p.district.zh ? p.district.zh : (p.district && p.district.en ? p.district.en : '未知');
...
function openModal(id) {
    ...
    currentModalImages = [];
    if (p.park_images) p.park_images.forEach(img => currentModalImages.push(buildImagePath(p.id, img)));
    if (p.equipment) {
        p.equipment.forEach(eq => {
            if (eq.images) eq.images.forEach(img => {
                const path = buildImagePath(p.id, img);
                if (!currentModalImages.includes(path)) currentModalImages.push(path);
            });
        });
    }
    if (currentModalImages.length === 0) currentModalImages.push('https://via.placeholder.com/800x600?text=No+Image');
    ...
    const equipmentHTML = (p.equipment && p.equipment.length > 0)
        ? p.equipment.map(e => {
            const zhName = getEquipmentName(e.type);
            const firstImg = (e.images && e.images.length > 0) ? buildImagePath(p.id, e.images[0]) : null;
